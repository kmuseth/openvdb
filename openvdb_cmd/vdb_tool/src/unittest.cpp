...
TEST_F(Test_vdb_tool, ActionForValuesWideStencil)
{
    // Regression: -forValues must handle kernels with more than 64 distinct
    // input variables. Each neighbor tap v(dx,dy,dz) is a separate Calculator
    // input variable, so a 5×5×5 = 125-tap stencil exposes 125 of them. An
    // earlier implementation copied inputs into a fixed float[64] buffer
    // guarded only by an assert (a no-op under NDEBUG), overflowing the stack
    // in release builds. The buffer is now sized dynamically per thread.
    using namespace openvdb::vdb_tool;
    using GridT = openvdb::FloatGrid;

    // 9×9×9 grid where each voxel stores its x-coordinate. Averaging all 125
    // taps in [-2,2]^3 leaves the value unchanged at interior voxels, because
    // the symmetric ±dx offsets cancel: mean of (i+dx) over dx∈[-2,2] is i.
    GridT::Ptr grid = GridT::create(0.0f);
    auto acc = grid->getAccessor();
    for (int i = 0; i <= 8; ++i)
      for (int j = 0; j <= 8; ++j)
    ...
    // Build the 125-variable averaging kernel: (v(-2,-2,-2)+...+v(2,2,2))/125.
    std::string kernel = "(";
    bool first = true;
    for (int dx = -2; dx <= 2; ++dx)
      for (int dy = -2; dy <= 2; ++dy)
        for (int dz = -2; dz <= 2; ++dz) {
            if (!first) kernel += "+";
            first = false;
            kernel += "v(" + std::to_string(dx) + "," + std::to_string(dy) +
                      "," + std::to_string(dz) + ")";
        }
    kernel += ")/125.0";

    {
      const std::string tmp = "data/test_wide_in.vdb";
      openvdb::io::File(tmp).write({grid});
      auto args = getArgs("vdb_tool -quiet -read " + tmp +
                          " -forOnValues " + kernel +
                          " -write data/test_wide_out.vdb");
      Tool t(int(args.size()), args.data());
    ...
    GridT::Ptr result;
    {
      openvdb::io::File f("data/test_wide_out.vdb");
      f.open();
      result = openvdb::gridPtrCast<GridT>(f.readGrid(f.beginName().gridName()));
    }
    ASSERT_TRUE(result != nullptr);
    auto racc = result->getConstAccessor();
    // Well-interior voxels (≥2 from every boundary) average back to i.
    for (int i = 2; i <= 6; ++i)
      for (int j = 2; j <= 6; ++j)
        for (int k = 2; k <= 6; ++k) {
            const float got = racc.getValue(openvdb::Coord(i, j, k));
            EXPECT_NEAR(static_cast<float>(i), got, 1e-4f)
                << "at (" << i << "," << j << "," << k << ")";
        }
}// ActionForValuesWideStencil
